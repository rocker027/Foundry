...
import { openDb, getStatusStats, closeDb, upsertSkill, insertLineage } from '../packages/hook-bridge/db.mjs';
import {
...
function usage() {
  process.stdout.write(`Foundry CLI v0.1.0
...
Usage:
  ...
  foundry simulate-recorder   # 測試 JSONL 寫入

Environment:
  FOUNDRY_MEMORY_ROOT   (default: ~/.foundry)
  FOUNDRY_SKILLS_ROOT   (default: ~/Documents/code/ai_coding_labs/skills)
...
function assertSkillsRootAllowed(skillsRoot, force = false) {
  ...
  const isDefault = resolved === defaultRoot;
...
function cmdPromote(slug, type = 'CAPTURED', force = false) {
  ...
  const srcDir = validation.dir;
  const destDir = join(skillsRoot, slug);

  try {
    assertPathWithinRoot(srcDir, PATHS.evolved());
  ...
  if (existsSync(destDir)) {
    ...
    process.stderr.write('Promote uses copy — manual merge required for updates.\n');
    process.exit(1);
  }
  ...
  if (isGitRepo(skillsRoot)) {
    ...
    const diffOut = join(srcDir, 'PROMOTE.diff');
    writeFileSync(diffOut, diff || '# New skill\n', 'utf8');
  }
  ...
  insertLineage(db, { childId: skillId, parentId: null, evolutionType: type });

  process.stdout.write(`Promoted ${slug} -> ${destDir}\n`);
  if (diff) {
    ...
    for (const jobPath of jobs) {
      ...
      try {
        if (job.type === 'analyze' && job.session_id) {
          const draft = analyzeSession(job.session_id);
          writeEvolvedDraft(draft);
          const db = openDb();
          upsertSkill(db, {
            skillId: crypto.randomUUID(),
            slug: draft.slug,
            origin: `session:${job.session_id}`,
            path: join(PATHS.evolved(), 'CAPTURED', draft.slug),
            state: 'draft',
          });
        }
        completeJob(jobPath, { status: 'done' });
        process.stdout.write(`Processed job ${job.job_id}\n`);
      } catch (err) {
        completeJob(jobPath, {
          status: 'failed',
